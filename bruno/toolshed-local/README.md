Bruno collection for the phase 2 Toolshed HTTP surface.
...
The collection persists `repositoryId`, `repositoryTriggerId`, and `webhookEventId` back into the environment after successful requests so the follow-up requests keep working without manual edits.
