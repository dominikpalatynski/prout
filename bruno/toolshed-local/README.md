Bruno collection for the phase 3B Toolshed HTTP surface.
...
The collection persists `repositoryId`, `repositoryTriggerId`, `webhookEventId`, and `runtimeEnvironmentId` back into the environment after successful requests so the follow-up requests keep working without manual edits.
