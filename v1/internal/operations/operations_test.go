...
func TestBuildPreviewDeleteSnapshotFreezesTargetRuntimeEnvironment(t *testing.T) {
	t.Parallel()

	githubPullRequestID := int64(9988)
	snapshotJSON, err := operations.BuildPreviewDeleteSnapshot(operations.TriggerIntentSnapshotInput{
		RepositoryID:        7,
		PullRequestID:       13,
		PRNumber:            42,
		GithubPullRequestID: &githubPullRequestID,
		...
		RuntimeEnvironmentType: operations.RuntimeEnvironmentTypePreview,
		TargetPRHeadCommitSHA:  "abc123",
		TargetRuntime: &operations.TargetRuntimeSnapshot{
			ID:               55,
			Status:           operations.RuntimeStatusPrepared,
			WorkspaceLocator: "runtime-environments/55",
		},
	})
	if err != nil {
	...
	if snapshot.Trigger.Type != automation.TriggerTypePreviewDeleteOnLabelPreviewRemoved {
		t.Fatalf("snapshot trigger type = %q, want %q", snapshot.Trigger.Type, automation.TriggerTypePreviewDeleteOnLabelPreviewRemoved)
	}
	...
	if snapshot.Target.RuntimeEnvironment.ID != 55 {
		t.Fatalf("snapshot target runtime_environment.id = %d, want 55", snapshot.Target.RuntimeEnvironment.ID)
	}
