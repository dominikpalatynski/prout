...
func TestPreviewDeleteStartsWithRuntimeTeardown(t *testing.T) {
	t.Parallel()

	step, err := InitialStepForOperation(TypePreviewDelete)
	if err != nil {
		t.Fatalf("InitialStepForOperation() error = %v", err)
	}
	if step.Name != StepRuntimeTeardown || step.State != StepStatePending {
		t.Fatalf("initial delete step = %#v, want runtime_teardown/pending", step)
	}
}
