...
func TestRegistryExposesSupportedEventFamiliesAndTriggerMappings(t *testing.T) {
	...
	if len(eventFamilies) != 4 {
		t.Fatalf("len(EventFamilies()) = %d, want 4", len(eventFamilies))
	}
	...
	if eventFamilies[2].TriggerTypes[0].Key != TriggerTypePreviewDeleteOnLabelPreviewRemoved {
		t.Fatalf(
			"EventFamilies()[2].TriggerTypes[0].Key = %q, want %q",
			eventFamilies[2].TriggerTypes[0].Key,
			TriggerTypePreviewDeleteOnLabelPreviewRemoved,
		)
	}
	if eventFamilies[3].TriggerTypes[0].Key != TriggerTypePreviewOnCommentPreview {
		t.Fatalf(
			"EventFamilies()[3].TriggerTypes[0].Key = %q, want %q",
			eventFamilies[3].TriggerTypes[0].Key,
	...
	deleteOperation, err := registry.OperationByKey(operations.TypePreviewDelete)
	if err != nil {
		t.Fatalf("OperationByKey(preview-delete) error = %v", err)
	}
	if deleteOperation.HandlerKey != HandlerPreviewDelete {
		t.Fatalf("OperationByKey(preview-delete) HandlerKey = %q, want %q", deleteOperation.HandlerKey, HandlerPreviewDelete)
	}
}
...
func TestEvaluateRepositoryTriggerMapsPreviewLabelRemovalToDeleteOperation(t *testing.T) {
	t.Parallel()
	...
	evaluation, err := registry.EvaluateRepositoryTrigger(sqlc.RepositoryTriggers{
		ID:           9,
		RepositoryID: 3,
		Type:         TriggerTypePreviewDeleteOnLabelPreviewRemoved,
		Enabled:      true,
	}, EventFamilyPullRequestUnlabeled, webhook.NormalizedEvent{
	...
	if evaluation.OperationType != operations.TypePreviewDelete {
		t.Fatalf("EvaluateRepositoryTrigger() OperationType = %q, want %q", evaluation.OperationType, operations.TypePreviewDelete)
	}
}
