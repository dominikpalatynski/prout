...
func (s *Server) processSupportedVerifiedDelivery(
	...
	err := s.store.Tx(ctx, func(q *sqlc.Queries, tx pgx.Tx) error {
		...
		for _, triggerRecord := range triggerRecords {
			...
			requestSpec, err := s.buildTriggerOperationRequestSpec(
				ctx,
				q,
				repositoryRecord,
				pullRequestRecord,
			...
			if err != nil {
				return markFailed(&repositoryID, err)
			}
			...
			operationRequest, err := q.InsertOperationRequest(ctx, sqlc.InsertOperationRequestParams{
				...
				TargetRuntimeEnvironmentID:      requestSpec.TargetRuntimeEnvironmentID,
				OperationType:                   evaluation.OperationType,
				Source:                          operations.SourceTrigger,
				Status:                          operations.StatusQueued,
				TargetPrHeadCommitSha:           requestSpec.TargetPRHeadCommitSHA,
				IntentSnapshotJson:              requestSpec.IntentSnapshotJSON,
...
type triggerOperationRequestSpec struct {
	IntentSnapshotJSON         []byte
	TargetPRHeadCommitSHA      string
	TargetRuntimeEnvironmentID *int64
}

func (s *Server) buildTriggerOperationRequestSpec(
	ctx context.Context,
	q *sqlc.Queries,
	repositoryRecord sqlc.Repositories,
	pullRequestRecord sqlc.PullRequests,
	triggerRecord sqlc.RepositoryTriggers,
	delivery webhook.Delivery,
	event webhook.NormalizedEvent,
	operationType string,
	operationDefinition automation.OperationDefinition,
) (triggerOperationRequestSpec, error) {
	...
	targetRuntimeEnvironment, err := resolveTriggerTargetRuntimeEnvironment(
		ctx,
		q,
		repositoryRecord.ID,
		pullRequestRecord.ID,
		operationType,
		operationDefinition.RuntimeEnvironmentType,
		event.PRHeadSHA,
	)
	if err != nil {
		return triggerOperationRequestSpec{}, fmt.Errorf("resolve trigger target runtime environment: %w", err)
	}
	...
	intentSnapshotJSON, err := operationDefinition.BuildTriggerIntentSnapshot(input)
	if err != nil {
		return triggerOperationRequestSpec{}, fmt.Errorf("build operation request snapshot: %w", err)
	}

	spec := triggerOperationRequestSpec{
	...
	if targetRuntimeEnvironment != nil {
		...
		spec.TargetRuntimeEnvironmentID = &targetRuntimeEnvironmentID
	}

	return spec, nil
}

func resolveTriggerTargetRuntimeEnvironment(
	ctx context.Context,
	q *sqlc.Queries,
	repositoryID, pullRequestID int64,
	operationType, runtimeEnvironmentType, targetPRHeadCommitSHA string,
) (*sqlc.RuntimeEnvironments, error) {
	if operationType != operations.TypePreviewDelete {
		return nil, nil
	}
	...
	runtimeEnvironment, err := q.GetLatestRuntimeEnvironmentByTarget(ctx, sqlc.GetLatestRuntimeEnvironmentByTargetParams{
		...
		TargetPrHeadCommitSha: targetPRHeadCommitSHA,
	})
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, nil
	...
	if !operations.RuntimeEnvironmentStatusIsActive(runtimeEnvironment.Status) {
		return nil, nil
	}
