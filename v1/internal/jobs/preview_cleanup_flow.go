...
func (w *OperationRequestWorker) handlePreviewDelete(
	ctx context.Context,
	operationRequest sqlc.OperationRequests,
) error {
	if operationRequest.TargetRuntimeEnvironmentID == nil {
		return w.completePreviewDeleteWithoutTarget(ctx, operationRequest)
	}

	return w.handlePreviewCleanupOperation(ctx, operationRequest, operations.RuntimeStatusDeleted)
}
...
func (w *OperationRequestWorker) handlePreviewCleanupSuperseded(
	...
	return w.handlePreviewCleanupOperation(ctx, operationRequest, "")
}

func (w *OperationRequestWorker) handlePreviewCleanupOperation(
	ctx context.Context,
	operationRequest sqlc.OperationRequests,
	completionRuntimeStatus string,
) error {
	for {
		...
		case operations.StepWorkspaceCleanup:
			if err := w.runCleanupWorkspaceStep(ctx, currentRequest, runtimeEnvironment, completionRuntimeStatus); err != nil {
				return err
			}
		default:
...
func (w *OperationRequestWorker) completePreviewDeleteWithoutTarget(
	ctx context.Context,
	operationRequest sqlc.OperationRequests,
) error {
	...
	return w.store.Tx(ctx, func(q *sqlc.Queries, _ pgx.Tx) error {
		return w.completeOperationRequestTx(ctx, q, operationRequest, nil, operations.OutcomeAlreadyDeleted, operations.StepStatus{
			Name:  operationRequest.CurrentStep,
			State: operations.StepStateCompleted,
		}, map[string]any{
			"resolution": operations.OutcomeAlreadyDeleted,
		})
	})
}
...
func (w *OperationRequestWorker) runCleanupWorkspaceStep(
	...
	return w.store.Tx(ctx, func(q *sqlc.Queries, _ pgx.Tx) error {
		completedRuntimeEnvironment, err := w.runtimeEnvironmentAfterCleanupStatusUpdate(
			ctx,
			q,
			runtimeEnvironment,
		...
		return w.completeOperationRequestTx(ctx, q, currentRequest, &runtimeEnvironment.ID, operations.OutcomeCleanupCompleted, operations.StepStatus{
			...
			State: operations.StepStateCompleted,
		}, runtimeEnvironmentStepDetails(completedRuntimeEnvironment, nil))
	})
}

func (w *OperationRequestWorker) runtimeEnvironmentAfterCleanupStatusUpdate(
	ctx context.Context,
	q *sqlc.Queries,
	runtimeEnvironment sqlc.RuntimeEnvironments,
	completionRuntimeStatus string,
) (sqlc.RuntimeEnvironments, error) {
	if completionRuntimeStatus == "" {
		return runtimeEnvironment, nil
	}

	currentRuntimeEnvironment, err := q.GetRuntimeEnvironmentByID(ctx, runtimeEnvironment.ID)
	if err != nil {
		return sqlc.RuntimeEnvironments{}, fmt.Errorf("reload runtime environment after cleanup: %w", err)
	}
	if !operations.RuntimeEnvironmentStatusIsActive(currentRuntimeEnvironment.Status) {
		return currentRuntimeEnvironment, nil
	}

	updatedRuntimeEnvironment, err := q.UpdateRuntimeEnvironmentStatus(ctx, sqlc.UpdateRuntimeEnvironmentStatusParams{
		ID:     currentRuntimeEnvironment.ID,
		Status: completionRuntimeStatus,
	})
	if err != nil {
		return sqlc.RuntimeEnvironments{}, fmt.Errorf("mark runtime environment %s: %w", completionRuntimeStatus, err)
	}

	return updatedRuntimeEnvironment, nil
}
