...
> **Dev:** "If the `preview` label is removed, which preview does it delete?"
> **Domain expert:** "It targets the latest active **Preview Environment** for the current **Pull Request Head Commit**. Historical superseded attempts are cleaned up automatically."
...
- A **Runtime Environment Status** for Phase 3 was unclear. Resolved: the initial status set is `preparing`, `prepared`, `failed`, `superseded`, and `deleted`.
...
- It was unclear how cleanup starts. Resolved: workspace cleanup can start automatically for a `superseded` **Runtime Environment** and can also be requested manually when the `preview` label is removed from a pull request.
- It was unclear how preview deletion resolves its target. Resolved: removing the `preview` label targets the latest active preview attempt for the current **Pull Request Head Commit**; older superseded attempts are not selected by that trigger.
...
- It was unclear whether automatic and manual cleanup share one operation kind. Resolved: trigger-driven `preview-delete` and automatic cleanup of superseded preview attempts are different **Operation Types** with separate audit meaning.
- It was unclear what cleanup work the current implementation includes. Resolved: the current implementation includes local technical cleanup of temporary materialization state, automatic cleanup of `superseded` preview attempts, and trigger-driven `preview-delete` when the `preview` label is removed.
...
- It was unclear whether explicit preview deletion should reuse the `superseded` runtime status. Resolved: when `preview-delete` finishes for a still-active target attempt, that **Runtime Environment** becomes `deleted` rather than `superseded`.
